...
void displayList(SortedListInterface<int>* intPtr)
{
	cout << "The sorted list contains " << endl;
   for (int pos = intPtr->getLength(); pos >= 1; pos--)
   {
      cout << intPtr->getEntry(pos) << " ";
   } // end for
	cout << endl << endl;
}  // end displayList
...
int main()
{
  ...
  do
  {
      cout << "What is the high score to add to the list? (Make sure score isn't negative!): ";
      cin >> score;

      if(score < 0 ) 
      {
         cout << "Score can't be negative! Now exiting program.\n";
         exit(0);
      }
      else
      {
         ...
         cout << "Would you like to add another score? (y/n): ";
         cin >> repeat;
      }
   } while (repeat == 'y' || repeat == 'Y');

   cout << "Here is the completed sorted list of high scores:\n";
   displayList(intList);
