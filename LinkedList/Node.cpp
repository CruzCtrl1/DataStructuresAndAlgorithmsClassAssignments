...
Node<ItemType>::Node() : next(nullptr), prev(nullptr) //Node is inheriting variables from next and from prev
{
...
Node<ItemType>::Node(const ItemType& anItem, Node<ItemType>* nextNodePtr, Node<ItemType>* prevNodePtr) :
                item(anItem), next(nextNodePtr), prev(prevNodePtr) //Added the prevNodePtr because
                                                                   //need to point to next and prev
{
} // end constructor
...
{
   return next; //Points to next item if iterated n times
} // end getNext
